...
- 测试代码会调用指定接口来进行测试，参赛队伍不能修改已有的接口，也不能删除已有的数据结构及数据结构中的变量，但是可以增添新的接口、数据结构、变量。
...
（1）void DiskManager::create_file(const std::string
            &path);
...
（2）void DiskManager::open_file(const std::string
            &path);
...
（3）void DiskManager::close_file(const std::string
            &path);
...
（4）void DiskManager::destroy_file(const std::string
            &path);
...
（5）void DiskManager::write_page(int fd, page_id_t page_no, const
            char *offset, int num_bytes);
...
（6）void DiskManager::read_page(int fd, page_id_t page_no,
            char *offset, int num_bytes);
...
(3) bool BufferPoolManager::find_victim_page(frame_id_t
            *frame_id);
...
(4) void BufferPoolManager::update_page(Page *page, PageId
            new_page_id, frame_id_t new_frame_id);
...
(5) bool BufferPoolManager::unpin_page(PageId page_id, bool
            is_dirty);
...
(1) std::unique_ptr<RmRecord>
            RmFileHandle::get_record(const Rid &rid, Context *context)
            const;
...
(2) Rid RmFileHandle::insert_record(char *buf, Context
            *context);
...
(3) void RmFileHandle::insert_record(const Rid &rid, char
            *buf);
...
(4) void RmFileHandle::delete_record(const Rid &rid,
            Context *context);
...
(5) void RmFileHandle::update_record(const Rid &rid, char
            *buf, Context *context);
