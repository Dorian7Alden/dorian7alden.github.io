...
本题目的输出要求写入数据库文件夹下的output.txt文件中，例如测试数据库名称为transaction_test_db，则在测试时使用./bin/rmdb
            transaction_test_db命令来启动服务端，对应输出应写入buid/transaction_test_db/output.txt文件中。
