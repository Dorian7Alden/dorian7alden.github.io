...
在数据库系统中，连接 (Join)
              操作的效率直接决定了复杂查询的响应速度。本题要求选手实现从基础的嵌套循环连接到索引嵌套循环连接的物理执行路径优化。

### 1.1 嵌套循环连接 (Nested Loop Join, NLJ)

基础的
              **Nested Loop Join**
              采用双层循环嵌套的逻辑。对于外表（驱动表）
              $R$
              中的每一行元组
              $r$，算子都会触发内表（被驱动表）
              $S$
...
- **复杂度**：若表规模分别为
                  ...
                  $O(N \times M)$。
- **执行特征**：内侧 Scan 节点表现为
                  `SeqScan`，被调用次数等于外表的行数。

### 1.2 索引嵌套循环连接 (Index Nested Loop Join, INLJ)

在本题中，当内表的连接属性存在唯一索引时，可以利用索引将内层的全量扫描优化为索引点查，即**Index Nested Loop Join**。

- **原理**：外表每读取一行，提取连接键值 (Join
                  Key)，直接作为参数传递给内层的索引扫描接口。系统仅检索与该键值匹配的特定行，而非扫描整张表。
- **复杂度**：时间复杂度优化为
                  $O(N \times \log M)$。对于大数据量，这种优化将带来量级的性能提升。
- **执行特征**：内侧 Scan 节点自动切换为
                  `IndexScan`，且内表算子由基础的全表遍历切换为基于索引的遍历。

### 1.3 本题约束与输出约定

本题考察两表及多表等值内连接，需要选手支持普通
              `SELECT ... FROM ... JOIN ... ON ...` 查询和
...
每个 `JOIN ... ON` 只包含表列之间的等值连接条件，例如
              `a.id = b.a_id`；多个连接条件可用
              `AND` 连接。

嵌套循环连接的执行形态固定为左深树：`表1 JOIN 表2 JOIN 表3`
              按
              `((表1 JOIN 表2) JOIN 表3)`
              执行。第一个表是初始外表/驱动表；每遇到一个新的
              `JOIN 右表 ON ...`，当前已经得到的左侧 Join
              结果作为外表/驱动输入，`右表` 作为内表/被驱动输入。例如
              `FROM a JOIN b ON a.id=b.a_id JOIN c ON a.id=c.a_id`
              的执行树为 `(a JOIN b) JOIN c`，其中第二层 Join
              的外表是 `a JOIN b` 的结果，内表是 `c`。
...
INLJ 阶段只会在各级 Join
              的右侧内表连接列上建索引，并且保证这些索引列取值唯一。

`EXPLAIN ANALYZE`
              的输出为文本形式的执行计划与运行时统计，不输出该查询本身的结果集。每个计划节点的
              `rows=N` 表示该节点本次执行时涉及的总行数。在 NLJ
              中右表作为内表会被左表重复扫描，因此右侧
              `SeqScan rows` 可以累计到
              `左表行数 × 右表行数`；在 INLJ
              中使用右表唯一索引进行点查时，右侧
              `IndexScan rows` 只统计命中的行数。

本题中 `EXPLAIN ANALYZE` 中的算子输出规范如下：

| 节点类型 | 格式 | 备注 |
| --- | --- | --- |
| Scan 节点 | Scan(table=表名, type=SeqScan, rows=行数) 或 Scan(table=表名, type=IndexScan, using_index=(列名),
                        rows=行数) | 叶节点。必须指明 type ；使用索引时必须指明索引列 using_index=(列名) 。 |
| Project 节点 | Project(columns=[表名1.列名1,表名2.列名2,...],
                        rows=行数) | SELECT 语句根节点一定是 Project
                      节点；此节点的列名输出时采用 表名.列名 形式，输出顺序为 表名.列名 按字典序排序，当需要保留全部列时输出 Project(columns=[*], rows=行数) 。 |
| Join 节点 | Join(tables=[表名1,表名2,...],
                        condition=[条件1,条件2,...], rows=行数) | 表名为该节点下所有表的集合，并按字母顺序排序；连接条件属于
                      Join 节点，多个连接条件按字典序排序。 |

计划树输出时，根节点行首不缩进，每深入一层在行首增加一个制表符
              `\t`，同一层级节点使用相同数量的
              `\t` 缩进，不使用空格代替制表符。Join
              节点的两个子树按照实际执行顺序输出，左侧子树表示外表或已经连接得到的中间结果，右侧子树表示内表。
...
***每个测试点都会分别执行无索引 NLJ 阶段和建索引后的 INLJ
                  阶段，检查普通 `SELECT` 结果和
                  `EXPLAIN ANALYZE` 计划输出。在
                  `test4` 中 当
                  $t_{INLJ} / t_{NLJ}$
                  < 0.5 时评定为性能达标***。
...
- `departments` 表（左表/驱动表）5 行，`dept_id`
                  为部门编号。
- `employees` 表（右表/被驱动表）5
                  行，每个员工属于唯一部门，`dept_id` 值各不相同。
- JOIN 结果为一一对应，共 5 行。
...
- 右侧 `Scan rows=25`：NLJ 中右表作为内表被左表 5
                  行重复全表扫描，共访问 `5 × 5 = 25` 行。
- 没有 `WHERE` 谓词时，不应输出
                  `Filter` 节点；连接条件应出现在
                  `Join(condition=[...])` 中。
...
- `Scan type` 变为
                  ...
                  `dept_id` 上的索引。
- 右侧
                  `Scan rows=5`：由于使用了唯一索引点查（Lookup），内表仅根据连接键精确读取匹配行，无需全表扫描。
- 右表访问行数从 `25` 降为
                  `5`，性能提升显著。
